...
public class Battle {
	...
	public static void battle() {
		...
		while (!playerOne.isDefeated() && !playerTwo.isDefeated()) { // finch� sono entrambi in vita
			...
			while (!golemOne.isDead() && !golemTwo.isDead()) { // finch� entrambi i Golem sono vivi
				...
				if (golemOne.getLife() != 0 && golemTwo.getLife() != 0) { // se non � morto stampa il danno
					Utility.printDemage(result, rockOne, rockTwo);
				} else if (golemOne.getLife() == 0) Utility.golemDead(playerOne); // altrimenti stampa chi � morto
